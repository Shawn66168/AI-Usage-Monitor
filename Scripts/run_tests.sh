...
printf '%s\n' '== Privacy scan =='
if grep -RInE '(\.credentials\.json|Session Storage|Cookies|transcript_path|history\.jsonl|payload\.(message|content)|Authorization: Bearer [A-Za-z0-9_-]+)' "$ROOT_DIR/Sources" "$ROOT_DIR/Tests"; then
  echo 'Privacy scan failed.' >&2
  exit 1
fi

if grep -RInE '(sk-ant-(api|admin)-[A-Za-z0-9_-]{12,}|sk-[A-Za-z0-9]{20,}|AIza[A-Za-z0-9_-]{20,})' "$ROOT_DIR" --exclude-dir=.build --exclude-dir=.git --exclude-dir=Build; then
